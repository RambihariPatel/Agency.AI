...
const ContactUs = () => {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "74ccbfcc-6f26-44d4-ac35-4c3aa69da722");
    ...
    if (data.success) {
      toast.success('Thanku for your submission')
      event.target.reset();
    } else {
      ...
      lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
        ...
        <form onSubmit = {onSubmit} className="grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-2xl w-full">
            ...
            <div className='sm:col-span-2'>
                ...
                <textarea name = "message" required rows={8} placeholder='Enter your message' className='w-full p-3 text-sm outline-none rounded-lg border border-gray-300 dark:border-gray-600' id='' />
            </div>

            <button type='submit' className='w-max flex gap-2 bg-primary text-while text-sm px-10 py-3 rounded-full cursor-pointer hover:scale-103 transition-all'>
                 Submit 
